...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        if self.in_layer=='filter':
            self.wiener_model = WienerNet(psf, device, batch_size=self.batch_size, fixed=self.fixed)
    ...
    def forward(self, x):
        encoder_outs = []
        if self.in_layer=='filter':
            x = self.wiener_model(x).type(torch.float32)[:, :, :, left:-left,top:-top]
            with torch.no_grad():
                np.save('stack_sim.npy', x.cpu().numpy())
            x = torch.cat((x[:, :, 0,...], x[:, :, 1,...], x[:, :, 2,...]), dim=1).squeeze(2)
#             with torch.no_grad():
#                 np.save('stack_sim.npy', x.cpu().numpy())
        ...
        if self.out_layer=='scaled_sigmoid':
            return 20*self.sigmoid(x)
        elif self.out_layer=='sigmoid':
            return 1/20 + self.sigmoid(x)
        elif self.out_layer=='softmax':
            return self.softmax(x)
        elif self.out_layer=='3Dvol':
            y = torch.cat([x[:, 0:3, ...], self.softmax(x[:, 3:self.out_channels, ...])], dim=1)
            return y
        elif self.out_layer=='depth':
            y = self.softmax(x)
            return y
        else:
            return x
